...
const uri = process.env.MONGODB_URI || "mongodb+srv://gopikajitesh:PASSWORD@leetq.5m2fq.mongodb.net/?retryWrites=true&w=majority&appName=LEETQ";
...
app.get("/random", async (req, res) => {
    if (!useMongo) return res.send({ message: "MongoDB not configured." });
    // ...existing code...
});
...
app.get("/", (req, res) => {
    res.send("Mini-Leet backend is running. Try /next or /random.");
});
